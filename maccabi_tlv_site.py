...
def handle_game(game: bs4.element.Tag) -> Event:
    ...
    event = {
        ...
        "extendedProperties": {"shared": {"url": official_game_page, "result": result}},
    }
    logger.info(event)
    return event


def fetch_games_from_maccabi_tlv_site(
    url: str, to_update_last_game: bool | None = False
) -> list[Event]:
    """
    ...
    if not to_update_last_game:
        ...
        logger.info(f'List of {len(games)} parsed games:"')
        ...
        for game in games:
            ...
            if game.find(text="מועד לא סופי") is None and game.find(text="לנוער") is None:
                ...
                logger.debug(f"Ignoring game without final date or a U19 game: {game}")
    else:
